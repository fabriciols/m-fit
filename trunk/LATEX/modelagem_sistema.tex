...
\item{void detectTransitions(Video, std::vector<Transition>)}
...
\item{bool validateTransition(long, std::vector<Transition>)}
...
NAO VOU MEXER , IVAN E MAURICIO QUE MEXXXXXXXXXXXAM
...
\item{virtual Frame* applyEffect(Frame)}
...
\item{double* calcDerivative(double* , int)}
...
\item{QImage* IplImageToQImage(uchar**, int*, int*, double, double)}
...
\hspace*{1.25cm}Classe que implementa o operador de morfologia matemática Dissolve como efeitos a ser aplicado.
\hspace*{0.65cm}\textbf{Métodos:}
\begin{enumerate}
...
\hspace*{0.65cm}Construtor que inicializa os atributos, e seta os valores
de inicio e fim do efeito.
...
\item{long getPosCurrent(void) const}
