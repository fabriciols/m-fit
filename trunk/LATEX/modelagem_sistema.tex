...
da reta, é feita uma análise, a fim de encontar possíveis fades.

\subsubsection{Detecta Fade-In \label{use_case_detecta_fadein}}

\hspace*{1.25cm}\textbf{Cenário Principal:} Feita a análise,
    cria-se os objetos do Fade-in.

\subsubsection{Fade-Out \label{use_case_detecta_fadeout}}

\hspace*{1.25cm}\textbf{Cenário Principal:} Feita a análise,
    cria-se os objetos do Fade-out.
...
\begin{itemize}
\item{Montagem do Ritmo Visual por Histograma}
\item{Verifica os próximos 7 frames}
\item{Cálculo da primeira derivada dos possíveis pontos de Dissolve}
\item{Verificação dos pontos resultantes da derivada com um limiar definido}
\end{itemize}
...
\subsubsection{Caso de uso: RV por histograma (RVH)}

\hspace*{1.25cm}Ver: \ref{use_case_rvh}

\subsubsection{Caso de uso: Verifica os 7 frames seguintes \label{use_case_7pontos}}

\hspace*{1.25cm}\textbf{Cenário Principal:} Verifica se estes 7 frames tem um comportamento crescente ou
decrescente, formando uma diagonal.
...
por Histograma do video, é calculada a derivada da diagonal demarcada como possivel dissolve.

\subsubsection{Caso de uso: Compara pontos da derivada com limiar definido \label{use_case_derivada_com_limiar}}

\hspace*{1.25cm}\textbf{Cenário Principal:} A partir da derivada do RHV é definido um limiar de corte para
separar transições do tipo dissolve.
...
\hspace*{1.25cm}Classe responsável por centralizar os métodos para
...
fade-in/fade-out e empilha o resultado no vector.
...
\hspace*{0.65cm}Calcula a média de luminância do Frame.
...
criação de uma imagem a partir de um Frame de entrada.
...
objeto Frame. Deve ser usada toda vez que for alterar a imagem de um
Frame.
...
concatenação dos frames, sendo que a ``soma'' só poderá ser feita
...
\hspace*{0.65cm}Função que retorna a maior luminância de um frame.
...
\hspace*{0.65cm}Habilita o botão ``Save''.

\item{void disableSaveButton()}

\hspace*{0.65cm}Desabilita o botão ``Save''.
...
\item{VR solicita à FRAME a média da luminância do frame corrente;}
...
Sistema entra no estado em que para cada ponto do Ritmo Visual por
Histograma, são verificados os próximos 7 pontos. Se estes possuírem
um comportamento crescente ou decrescente, o primeiro deles será
considerado um possível local de ocorrência de Dissolve.}
...
possíveis pontos de Dissolve determinados no estado anterior.}
...
definido, este será considerado um ponto de ocorrência de Dissolve.}
