...
\begin{enumerate}
    ...
    verifica se o ponto apontado como transição de corte não se trata de um fade
    através da análise da média da luminância dos próximos frames e dos frames anteriores.
    Se esta for muito baixa, significa que os próximos frames ou os frames anteriores são pretos, ou bastante
...
\hspace*{1.25cm}Como visto na Seção \ref{ed_fadein}, o fade-in é uma
transição que parte de tomadas mais escuras para tomadas mais
claras, enquanto o fade-out, visto na Seção \ref{ed_fadeout}, possui
um efeito inverso, partindo de tomadas mais claras para tomadas mais
escuras.

\hspace*{0.65cm}Devido à similaridade entre estas duas transições, a
metodologia utilizada para a detecção de ambas será semelhante, como
pode ser visto na Figura \ref{img_metfades}. Primeiramente será
mostrada a metodologia do fade-in e posteriormente serão comentadas
as diferenças da mesma em relação ao fade-out.
...
\begin{enumerate}
    \item{\textbf{Vídeo:} É feita a entrada do vídeo no qual será realizada a detecção dos fade-in.}
    ...
    \begin{figure}[h|top]
        ...
        \caption{RVH derivado de uma sequência que contém uma transição do tipo fade-in, seguida de um fade-out.}
          \label{img_vrhd_fade_in_out} Fonte: Autor.
    \end{figure}

        Tendo as derivadas calculadas, é feita a análise, a fim de encontrar padrões de fade-in e fade-out,
          os quais podem ser vistos na Figura \ref{img_vrhd_fade_in_out}. Nota-se que em ambos os casos de fade,
          fica evidente que um fade é representado por um intervalo de variação na derivada do ritmo-visual.

        Então o array de derivadas é percorrido ponto a ponto, em busca de intervalos onde a derivada sofre uma variação, como nas figuras citadas acima.
        Para que o sistema comece a avalizar um intervalo de variação esta precisa atender as seguintes condições:

        \begin{enumerate}
        \item{ Sua derivada deverá sofrer uma variância (não deve ser constante); }
        \item{ Esta variância não deve ser abrupta (Pois pode evidenciar um CORTE);}
        ...
        O ponto final desta variação é determinado quando a derivada para de variar e se torna constante novamente.

        Sob posse do intervalo da variação, o sistema checa as seguintes condições:

        \begin{enumerate}
        \item{ O intervalo de variação deve conter mais do que 10 Frames, visto que valores abaixo deste limiar podem evidenciar um Corte;}
        \item{ O intervalo não deve ter em seu ponto final uma variação muito abrupta, podendo assim, evidenciar um Corte}
        \end{enumerate}

        Assim, fica definido que uma variação é de fato um fade.
        Para ser feita a diferenciação entre um Fade-in e um Fade-out, são adotados os seguintes critérios:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item{Cálculo da Diagonal Principal deste Frame;}
        \item{Obtenção do Histograma desta diagonal;}
        \item{Cálculo da Variância do Histograma (MAURICIO - NÃO É DOS FRAMES DO VÍDEO?).}
    \end{enumerate}

    \hspace*{0.65cm}Tais passos se repetem seqüencialmente do primeiro,
    até o último frame do vídeo.

    \item{\textbf{Verifica pontos de dissolve com 7 frames seguintes:} A partir dos pontos do
    Ritmo visual por histograma, são verificados os próximos 7 frames (MAURÍCIO - PROXIMOS 7 FRAMES? EM RELAÇÃO
    À QUE!?!).
    Caso estes frames estejam em ordem crescente ou decrescente, este frame é um possível ponto de dissolve.
    MAURÍCIO - ISTO AQUI NÃO ESTÁ CLARO.NÃO DÁ PRA ENTENDER NADA... REESCREVAM}

    \item{\textbf{Derivada dos possíveis pontos de Dissolve:} Para os possíveis pontos de Dissolve,
    a derivada destes pontos é calculada, comparando os resultados obtidos a um limiar.
    MAURÍCIO, TAMBÉM NÃO FICOU MUITO CLARO;}

    \item{\textbf{Detecção do Dissolve:} Caso os pontos em questão forem todos menores que o limiar,
    o primeiro ponto será indicado como início do Dissolve;}

    \item{\textbf{Verificação de frames escuros nos pontos detectados:} Com base nas detecções,
    verificamos se o nível de luminância (MAURICIO - LUMINANCIA DE QUEM???) é proximo ao preto, caso positivo,
    ele (MAURICIO - ELE!? ELE QUEM?) será descartado, pois isto pode indicar que esta transição representa um
    fade e não um dissolve;}
