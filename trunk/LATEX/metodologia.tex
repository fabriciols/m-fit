...
\begin{enumerate}
    ...
    \item{\textbf{Verifica comportamento do RVH:} A partir dos pontos do
    Ritmo visual por histograma, é analisado para cada ponto do gráfico, se os 7 pontos seguintes seguem uma 
    ordem crescente ou decrescente, formando uma diagonal, este frame é um possível ponto de dissolve.

    \item{\textbf{Derivada dos possíveis pontos de Dissolve:} Com os possiveis pontos de dissolve, é feito uma
    análise da derivada desta diagonal para cada ponto do gráfico do RVH, caso esta derivada não ultrapassar
    o limiar definido, este é um ponto canditado ao dissolve.

    \item{\textbf{Retirar transições do tipo fade:} Com base nos pontos de possiveis transições do tipo Dissolve,
    verificamos o nível de luminância mais baixo da diagonal do RVH em questão, caso este frame estiver dentro do
    limiar de fade definido ele será descartado, pois isto pode indicar que esta transição representa um fade e 
    não um dissolve;}
