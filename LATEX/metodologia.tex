...
\begin{enumerate}
    ...
    Histograma, serão verificados os próximos 7 pontos de acordo com
    observações realizadas em vídeos de teste em que ocorrem o Dissolve. Nestes
    testes foi constatado que em média, são necessários 7 pontos.
    Se estes pontos possuírem um comportamento crescente ou decrescente, o primeiro
    deles será considerado um possível local de ocorrência de
    Dissolve.}

    \item{\textbf{Derivada dos possíveis pontos de Dissolve:} Será
    calculada a primeira derivada dos trechos em que há uma possível
    ocorrência de Dissolve, a qual será submetida a um processo de
    limiarização. Se todas as derivadas de um determinado trecho
    estiverem abaixo do limiar definido, este será considerado um
    ponto de ocorrência de Dissolve.}

    \item{\textbf{Retirar transições do tipo fade:} Será feita a
    análise da média do nível de luminância dos Frames pertencentes
    ao trecho apontado como Dissolve. Caso um deles possua uma média
    de luminância próxima de zero, significa que o ponto detectado pode não ser um Dissolve
    e sim um Fade. Desta forma o ponto é ignorado.}
