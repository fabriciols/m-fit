...
\hspace*{0.65cm}Considerando os resultados visto na Tabela \ref{tab:resultados_fade_teste}, o resultado é 100\% ``Perfeito'', com todas as reais transições
detectadas, e as não-transições, não detectadas.

\hspace*{0.65cm}Conclui-se que, em Vídeos onde o ``fade'' é criado seguido
algumas regras, a detecção tem uma taxa de acerto de 100\%.
...
chegamos a uma taxa de 100\% ``True'' positivo, e de 99\% de ``True'' Negativo.
Assim a performance da detecção fica em 99\% de acerto, o qual é um exelente valor para tal experimento.
...
\begin{enumerate}
\item{ Esta transição tem menos de 10 frames em sua composição, sendo este o limiar de corte. }
...
\hspace*{0.65cm}Na Figura \ref{img:vrh_batman} é demonstrado um trecho do Ritmo Visual por Histograma do vídeo Batman.avi, onde
as transições detectadas estão demarcadas com um traço vertical.
...
\hspace*{0.65cm}Nesta imagem é possível verificar, que todo fade segue o padrão de um platô seguido de um variação tendendo
ao preto (valor de luminância), no caso do ``fade-out'', a transição começa no preto e vai variando até a chegar em um platô, no caso
do ``fade-in''.
Este funcionamento pode ser entendida de melhor forma na Seção \ref{met_fade}.

\hspace*{0.65cm}Na Figura \ref{img:vrhd_batman} é demonstrado um trecho da derivada do Ritmo Visual por Histograma do vídeo Batman.avi, onde as transições detectadas estão demarcadas com um traço vertical.
...
\hspace*{0.65cm}A figura \ref{img:rvh_teste} contém o ritmo visual junto com o seu respectivo gráfico, 
onde indica os pontos de dissolve detectados a partir da diagonal do gráfico (D1 e D2).
...
\hspace*{0.65cm}Para este vídeo, a taxa de acerto foi de 0\%. Uma grande porção de 
cenas contendo fumaças, nuvens e explosões se confundem com uma transição do tipo 
dissolve pois fazem sobreposições de frames, resultando em uma taxa de acerto não favorável.
...
a taxa de acerto é de 21,42\%. Devido ao grande número de explosões detectadas nas 
cenas, elas se confundem com uma transição do tipo dissolve pois fazem sobreposições 
de imagens, contribuindo assim para uma baixa taxa de acerto.
