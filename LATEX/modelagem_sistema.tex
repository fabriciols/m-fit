...
\hspace*{1.25cm}\textbf{Cenário Principal:} O usuário solicita ao
sistema que seja realizada a detecção de transições do tipo Fade.
...
variações na curva das derivadas, a fim de encontrar possíveis Fades
e classificá-los entre Fade-In e Fade-Out.
...
7 frames, a partir do primeiro frame da região candidata a ser um
dissolve, possuem um comportamento crescente ou decrescente.
...
possível ocorrência de dissolve.
...
RHV é definido um limiar para separar transições do tipo dissolve e
fade.
...
\subsection{Classe: Render\label{classe_Xml}}
...
\hspace*{0.65cm}Começa o provesso de renderização, chamando a thread
responsável.

\item{void writeFrame()}

\hspace*{0.65cm}Escreve um frame na estrutura do vídeo.

\end{enumerate}

\subsection{Classe: RenderThread\label{classe_Xml}}
...
\hspace*{0.65cm}Envia para a janela a pocentagem atual do processo.
...
\item{VR cria objeto HIST, criando o histograma do frame corrente;}
...
estado são detectados os Fades presentes no vídeo.
...
Fades e os classifica em Fade-In ou Fade-Out.}

\item{\textbf{Verificando duplicidade}

Sistema faz a verificação se o ponto apontado como transição de Fade
...
Também é aplicado um limiar para distinção entre um Dissolve e um
Fade.}

\item{\textbf{Verificando duplicidade}

Sistema faz a verificação se o ponto apontado como transição de
Dissolve já não foi apontado anteriormente como ponto de ocorrência
