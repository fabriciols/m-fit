...
\centering
    ...
    Fonte: Adaptado de "OpenCV Samples".
    \label{img_sobel}
\end{figure}
...
\begin{enumerate}
\item Taxa de Erro (SNR)
\item Localização (L)
\item Resposta
\end{enumerate}

\hspace*{0.65cm}Onde a Taxa de Erro  representa a eficiência do filtro
em relação a falsos positivos (detecção de falsas bordas) e falsos negativos
(bordas verdadeiras não detectadas), ou seja, o filtro deve detectar o máximo
de bordas verdadeiras existentes na imagem.

\hspace*{0.65cm}O critério de localização, como o nome já diz, está ligado
à localização real da borda na imagem e àquela apontada pelo filtro. Quanto
menor a distância entre a borda detectada da borda real, melhor será o
filtro.

\hspace*{0.65cm}O último dos critérios corresponde à quantidade de respostas
geradas pelo filtro em relação à uma única borda. Para cada borda existente
na imagem, o filtro deve apontar somente uma resposta.

\hspace*{0.65cm}Para atender a estes critérios, são adotados
adotados, além de processos de convolução (Seção
\ref{filtragem_espacial}), processos de Supressão não Máxima e
Histerese.

\hspace*{0.65cm}A Supressão não Máxima consiste (\cite{GIOVANE_2004}) na eliminação dos pixels
da região da borda que não possuírem magnitude máxima, onde a magnitude dos pixels
é dada por:
...
\hspace*{0.65}Onde $I_x$ é a imagem convoluída na direção X e $I_y$ é a imagem convoluída na
direção Y.

\hspace*{0.65}Como visto no início desta seção, as bordas são representadas por pixels
que possuem gradiente de alta magnitude. Devido à ruídos presentes na imagem, após os processos de
convolução da imagem, podem haver diversos pixels representando um
único ponto de borda verdadeiro. Porém, aquele que possuir o
gradiente com maior magnitude é o candidato mais forte para
representar a borda. Através do processo de supressão não máxima, os
outros pixels (de magnitude não máxima) são eliminados, restando
somente 1 pixel representando 1 ponto de borda, o que melhora a performance do filtro
e atende ao terceiro critério citado acima, visto que as bordas serão representadas
por linhas de apenas 1 pixel.

\hspace*{0.65cm}Para evitar descontinuidades das bordas detectadas, é utilizado
o processo de histerese (\cite{GIOVANE_2004}), ou limiarização adaptativa. Este processo consiste em
aplicar duas vezes o processo de limiarização em uma mesma imagem, com a utilização de
dois limiares, $T_{1}$ e $T_{2}$, onde $T_{1} = 2T_{2}$ ou $T_{1} = 3T_{2}$.
A limiarização se dá da seguinte forma:
...
\centering
    ...
    \begin{minipage}[b]{7.0cm}
        ...
        (d)
    \end{minipage}
    \caption{a) Figura original; b) Limiar 150; c) Limiar 100; d) Limiar 50} Fonte: Adaptado de "OpenCV Samples".
    \label{img_limiar}
\end{figure}
